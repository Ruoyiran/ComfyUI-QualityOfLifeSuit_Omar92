...
def get_config():
    ...
    try:
        # open config file
        config_path = os.path.join(SUIT_DIR, "config.json")
        with open(config_path, 'r') as f:  # Open the file and read the API key
            config = json.load(f)
        return config
    except Exception as e:
        print("Read config file failed:", e)
    return {}
...
def get_openai_api_key():
    return get_openai_config().get("api_key", "")


def get_azure_api_endpoint():
    return get_openai_config().get("azure_api_endpoint", "")


def get_gpt_models():
    return get_openai_config().get("models", [])


def is_azure_openai():
    return get_openai_config().get("azure_api_version", "") != ""
...
def create_openai_client():
    if is_azure_openai():
        ...
        return AzureOpenAI(
            ...
            azure_endpoint=get_azure_api_endpoint(),
        )
    else:
...
def chat_completion_with_messages(messages, model="gpt-3.5-turbo"):
...
def chat_completion(system_message, prompt, model="gpt-3.5-turbo"):
    messages = []
    if system_message is not None and system_message != "":
        messages.append({"role": "system", "content": system_message})
    messages.append({"role": "user", "content": prompt})
    return chat_completion_with_messages(messages, model)
...
class O_ChatGPT_O:
    ...
    def fun(self, model, prompt, behaviour, seed):
        ...
        if behaviour == "description":
            init_message = get_init_message(False)
        else:
            init_message = get_init_message(True)

        completion = chat_completion(init_message, prompt, model)
...
class O_ChatGPT_medium_O:
    ...
    def fun(self, model, prompt, initMsg, seed):
        ...
        # Get the API key from the file
        completion = chat_completion(initMsg, prompt, model)
        # Get the answer from the chat completion
        answer = completion.choices[0].message.content
        return (answer,)  # Return the answer as a string
...
def loadFonts():
    ...
    try:
        ...
        for file in os.listdir(fonts_filepath):
            if file.endswith(".ttf") or file.endswith(".otf") or file.endswith(".ttc") or file.endswith(
                    ".TTF") or file.endswith(".OTF") or file.endswith(".TTC"):
                fonts.append(file)
    except:
        fonts = []
...
class Text2Image_O:
    ...
    def create_image_new(self, text, font, size, font_R, font_G, font_B, font_A, background_R, background_G,
                         background_B, background_A, width, height, expand, x, y):
